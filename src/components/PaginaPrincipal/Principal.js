...
import { useNavigate } from 'react-router-dom'; // Hook para redireccionamiento
import './Principal.css'; // Estilos CSS específicos del componente

const Principal = () => {
  // Estados para emociones, nivel de intensidad, texto del diario, entradas guardadas y fecha actual
  ...
  // Lista de emociones con sus nombres, emojis y estilos
  const emotions = [
  ...
  // Al cargar el componente, valida que el usuario esté logueado
  useEffect(() => {
  ...
  // Función que guarda la entrada emocional del día
  const handleSaveEntry = async () => {
    if (!selectedEmotion) return; // Validar que se haya seleccionado una emoción
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          if (response.ok) {
            // Si se guardó exitosamente, limpiar campos y mostrar éxito
  ...
  const renderCalendar = () => {
    ...
    // Agrega espacios vacíos al inicio del mes
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(<div key={`empty-${i}`} className="calendar-day empty"></div>);
    }

    // Agrega los días con marcas si ya tienen entradas
  ...
  // Componente visual para seleccionar el nivel de emoción
        ...
        <div className="dashboard-container">
          ...
          <div className="emotion-panel">
            <div className="principal-card">
              ...
              {/* Si se seleccionó una emoción, muestra el nivel e input */}
              {selectedEmotion && (
                <>
